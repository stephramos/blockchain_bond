...
'''
issue interest tokens, par value and create scrow acct for token transactions
'''

def check_funds(my_address):
    '''
    '''
    ...
    for asset in account_info.get('assets'):
        print("Account balance: {} asset id {}".format(asset.get('amount'), asset.get('asset-id')) + "\n")
...
#--------------------------------------------------------------------
# SETTING PARAMETERS
closure =  20107300 + 20  # 20 + CURRENT BLOCK NUMBER - to claim interest right away
first_coupon_payment = closure + 1 # the first block from which a bondholder is allowed to claim interest
proj_name = "algotest" # the name used for token issuance
url= None
payment_id = "YOUR PAYMENT TOKEN"
par = 5 # the par value of bond as measured in units of payment_id
coupon = 1 # the coupon value of bond as measured in units of payment_id
vol = 1000 # total number of bond available
total_payments = 5 # how many interest payments in total
amt = 1 # number of bonds to be purchased by buyer
...
interest_id, par_id, escrow_result, escrow_id= main_pub(passphrase=pub_pass, proj_name=proj_name,
                                                        vol=vol, url=url, par=par, coupon=coupon, payment_id=payment_id, closure=closure,
